...
public class HintController {
  ...
  public ResponseEntity<List<HintResponse>> getHintsByLevel(
      @PathVariable Long questId, @PathVariable Long levelId) {
    return ResponseEntity.status(HttpStatus.OK).body(hintService.getHintsByLevelId(levelId));
  }
  ...
  public ResponseEntity<HintResponse> getHintById(@PathVariable Long hintId) {
    return ResponseEntity.status(HttpStatus.OK).body(hintService.getHintById(hintId));
  }
  ...
  public ResponseEntity<HintResponse> updateHint(
      ...
      @Valid @RequestBody CreateHintRequest request,
      Authentication auth) {
    return ResponseEntity.status(HttpStatus.OK).body(hintService.updateHint(hintId, request, auth));
  }
