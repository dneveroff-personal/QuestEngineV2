...
class QuestControllerTest {
    ...
    void getQuestById_returnsQuest_whenQuestExists() throws Exception {
        when(questService.getQuestById(eq(1L)))
                .thenReturn(questResponse);
    ...
    void updateQuest_returnsUpdatedQuest_whenRequestIsValid() throws Exception {
        QuestResponse updatedResponse = new QuestResponse(
                1L,
                "Updated Quest",
                "Updated Description",
                QuestType.SINGLE,
        ...
        mockMvc.perform(put("/api/quests/1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"title\":\"Updated Quest\",\"description\":\"Updated Description\",\"type\":\"SINGLE\"}"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.id").value(1))
    ...
    void createQuest_returnsConflict_whenUserIsNotAuthorOrAdmin() throws Exception {
        when(questService.createQuest(any(CreateQuestRequest.class), any()))
                .thenThrow(new ForbiddenOperationException("Создавать квесты могут только AUTHOR или ADMIN"));
