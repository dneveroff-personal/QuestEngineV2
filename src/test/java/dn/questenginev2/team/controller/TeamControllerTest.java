...
class TeamControllerTest {
    ...
    @BeforeEach
    void setUp() {
        teamResponse = new TeamResponse(1L, "Test Team", "Test User", Instant.now(), Collections.emptyList());
        joinResponse = new TeamJoinResponse(1L, "requester", JoinRequestType.JOIN_REQUEST, Instant.now());
    ...
    void createJoinRequest_returnsTrue_whenRequestIsValid() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(post("/api/teams/1/request"))
                .andExpect(status().isCreated())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
    ...
    void createJoinRequest_returnsTrue_whenUsernameProvided() throws Exception {
        // Arrange
        when(teamService.createJoinRequest(any(), eq(1L), eq("inviteduser")))
                .thenReturn(true);

        // Act & Assert
        mockMvc.perform(post("/api/teams/1/request")
                        .param("username", "inviteduser"))
                .andExpect(status().isCreated())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$", is(true)));
    }
    ...
    void approveRequest_returnsTrue_whenRequestIsValid() throws Exception {
        // Arrange
        when(teamService.approveRequest(eq(1L), any()))
                .thenReturn(true);

        // Act & Assert
        mockMvc.perform(post("/api/teams/requests/1/approve"))
    ...
    void rejectRequest_returnsTrue_whenRequestIsValid() throws Exception {
        // Arrange
        when(teamService.rejectRequest(eq(1L), any()))
                .thenReturn(true);

        // Act & Assert
        mockMvc.perform(post("/api/teams/requests/1/reject"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$", is(true)));
    }
    ...
    void leaveTeam_returnsTrue_whenRequestIsValid() throws Exception {
        // Arrange
        when(teamService.leaveTeam(any()))
                .thenReturn(true);
    ...
    void transferCaptain_returnsTrue_whenRequestIsValid() throws Exception {
        // Arrange
        when(teamService.transferCaptain(eq(2L), any()))
                .thenReturn(true);

        // Act & Assert
        mockMvc.perform(post("/api/teams/transfer-captain/2"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$", is(true)));
    }
    ...
    void getJoinRequests_returnsRequests_whenUserIsAuthenticated() throws Exception {
        // Arrange
        when(teamService.getJoinRequests(any()))
                .thenReturn(Collections.singletonList(joinResponse));

        // Act & Assert
        mockMvc.perform(get("/api/teams/requests"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
    ...
    void getMyTeam_returnsTeam_whenUserIsInTeam() throws Exception {
        // Arrange
        when(teamService.getMyTeam(any()))
                .thenReturn(teamResponse);
    ...
    void getTeamMembers_returnsMembers_whenTeamExists() throws Exception {
        // Arrange
        when(teamService.getTeamMembers(eq(1L)))
                .thenReturn(Collections.singletonList(memberDto));
    ...
    void approveRequest_returnsNotFound_whenRequestNotFound() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(post("/api/teams/requests/999/approve"))
                .andExpect(status().isNotFound())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
