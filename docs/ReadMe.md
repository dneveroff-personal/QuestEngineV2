...
**Следующий шаг проекта** (по вашему указанию "документация → архитектура → код"): пройтись по списку "Решения, требующие ADR" ниже, оформить фактические ADR-документы в `03-architecture/adr/` для каждого пункта, и только затем переходить к архитектурному проектированию и реализации `CodeSubmission`/`HintProgress`/планировщика.
...
## Решения, требующие ADR (найдены при написании раздела "Security / Quality / Vision")

12. **Отзыв JWT раньше истечения срока (24 часа)** — сейчас невозможен технически; требуется решение, критичен ли сценарий "забанить читера посреди игры". См. `05-security/threat-model.md`.
13. **Внедрение rate limiting** — в проекте нет ни одной библиотеки для этого; минимум требуется для `/auth/login`, а в перспективе и для будущего ввода кодов. См. `05-security/threat-model.md`.
14. **Порог покрытия тестами для CI** (`jacocoTestCoverageVerification`) — JaCoCo подключён, но не проваливает сборку при недостаточном покрытии. См. `07-quality/testing-strategy.md`.
15. **Инъекция `Clock` как обязательное правило** для любого нового time-dependent сервиса (уже частично сделано в `LevelProgressServiceImpl`/`QuestProgressServiceImpl`, но не везде, включая `QuestRegistrationServiceImpl`). См. `07-quality/testing-strategy.md`.
16. **Целевая аудитория и формат игры (офлайн/онлайн)** — не определены, влияют на приоритеты NFR. См. `00-vision/product-vision.md`.
