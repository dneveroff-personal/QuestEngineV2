...
## Job 3 — Hint Auto-Reveal (реализовано, только REGULAR)

`01-domain/hint-progress.md`, ADR-0020, ADR-0021.

### Триггер
Для каждого `LevelProgress.status == ACTIVE`, для каждой **REGULAR** подсказки его уровня: `LevelProgress.openedAt + Hint.delaySeconds <= now()` и подсказка ещё не показана.

ADR-0021: `BONUS`/`PENALTY`-подсказки этот job **не трогает** — они становятся доступны команде (видна информация о наличии и типе, без содержимого), но переходят в показанные только по явному действию команды (`POST .../hints/{hintId}/take`), не автоматически.

### Действие
`HintRevealScheduler.revealDueHints()`: находит все `LevelProgress` в статусе `ACTIVE`, для каждого — REGULAR-подсказки уровня, для каждой ещё не показанной и с истёкшей задержкой создаёт `HintProgress`.

### Отличие от Job 1/Job 2

Здесь **нет конкурирующего пути** — показ REGULAR-подсказки не может произойти никаким другим способом, кроме этого планировщика (в отличие от Job 2, который конкурирует с `CodeSubmission` за один и тот же `LevelProgress`, Сценарий 5). Идемпотентность обеспечивается уникальным индексом `(level_progress_id, hint_id)` и перехватом нарушения — defense-in-depth, а не разрешение гонки между двумя разными путями. Тот же принцип (уникальный индекс + идемпотентная обработка нарушения) используется и в явном взятии BONUS/PENALTY командой — там конкурирующий путь тоже отсутствует (взять может только сама команда, никакой другой процесс), но два разных участника одной команды могут одновременно нажать «взять» — идемпотентность защищает от двойного эффекта в этом случае.
