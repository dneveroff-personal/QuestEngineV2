...
| **Team (частично)** | Read-only просмотр своей команды (состав, капитан) + создание команды, если её ещё нет (`getMyTeam` 404 → трактуется как валидное состояние, не ошибка). Вступление/поиск/передача капитанства — ещё не реализовано, следующий шаг. |
...
3. **Team — доделать** — 🟡 частично готово (просмотр + создание). Осталось: поиск команды, вступление по заявке, подтверждение капитаном, передача капитанства, выход из команды. Backend полностью готов (раздел 2.2).
