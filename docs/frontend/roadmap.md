...
| **Profile** | Личная информация (`publicName`/`username`/`role` — из сессии и JWT, без обходных путей; `email`/`createdAt` — тем же честным резолвом через `/api/users/search`, что и `useMyAuthorId`, с явным "недоступно" при неоднозначности), список авторских квестов (переиспользует `useMyAuthoredQuests` из Author-CRUD). История/статистика — не реализованы, см. §4.1/§4.4. |
...
- **Profile** — `email`/`createdAt` реализованы через тот же обходной
  резолв, что и `userId` в Team/Author (`features/profile/useMyProfile.ts`:
