...
4. **Author — CRUD раздел** — backend полностью готов (раздел 2.3), самая объёмная часть по количеству форм. Следующий логичный шаг.
5. **Profile** — частично (раздел 4.3) до появления `GET /api/users/me` (email/createdAt), роль уже доступна через JWT.
