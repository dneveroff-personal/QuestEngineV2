...
| **Game Mode (частично)** | Вход в игру (WAITING → RUNNING), ввод кода (`CodeSubmitForm`, реакция на `CORRECT_MAIN/BONUS/PENALTY/INCORRECT`), список показанных подсказок (`ShownHintsList`, polling 5с — push для этого не предусмотрен, ADR-0014 касается только статистики). Экран отдельный, вне `RootLayout` (`/quests/:questId/play`). **Не показывает название/содержание уровня и таймер автоперехода** — backend их нигде не отдаёт (см. 4.7). Реализовано на реальном API, не на моках. |
| **Team — упрощение (0.6.11)** | `TeamMemberDto` теперь отдаёт `userId` напрямую — `transferCaptain` больше не резолвит его через `/api/users/search` (было реальным риском неоднозначности, см. было в 4.5, теперь исправлено). |
| **Profile** | Личная информация (`publicName`/`username`/`role` — из сессии и JWT, без обходных путей; `email`/`createdAt` — тем же честным резолвом, что и `useMyAuthorId`, оба используют общий `features/auth/useMyResolvedUser.ts` — вынесен из дублирования при ревью), список авторских квестов (переиспользует `useMyAuthoredQuests` из Author-CRUD). История/статистика — не реализованы, см. §4.1/§4.4. |
| **Каркас страниц** | `RootLayout` (навигация, имя пользователя, выход). |
...
> **Исправление (0.6.11):** до этой версии здесь было ошибочно написано,
> что ввод кода и подсказки во время игры не существуют вовсе. Это было
> неточно — `CodeSubmissionController`/`QuestProgressController`
> существовали и раньше, просто не были проверены напрямую при первом
> составлении этого документа. См. раздел 1 ("Game Mode") — реализовано,
> но с существенной оговоркой в 4.7.
...
| **Manual time adjustment (бонус/штраф вручную)** | Появился пакет `bonuspenalty/` (0.6.11) — но только entity (`ManualTimeAdjustment`, `TimeAdjustmentType`), ни сервиса, ни контроллера ещё нет. Пока нечего интегрировать. |
...
- **Profile** — `email`/`createdAt` реализованы через тот же обходной
  резолв, что и `userId` в Team/Author (общий `features/auth/useMyResolvedUser.ts`, вынесен туда при рефакторинге — раньше был продублирован в authoring и profile отдельно:
  `/api/users/search?username=свой_username`, при неоднозначном результате
  честно показывает "недоступно", а не гадает). Работает на практике, но
  остаётся обходным путём вокруг отсутствующего `GET /api/users/me`, а не
  постоянным решением — при росте базы пользователей неоднозначность
  (несколько похожих username) будет возникать всё чаще.
...
### 4.7 КРИТИЧНО для Game Mode — нет способа узнать текущий уровень команды

Самая важная находка при реализации Game Mode (0.6.11). `LevelProgressResponse`
(`levelId`, `levelTitle`, `status`, `openedAt`, `completedAt`, `autoTransitionAt`
— именно то, что нужно для таймера из `architecture.md` §18) полностью
готов на backend и используется внутри `LevelProgressService`, но **ни
один контроллер его не возвращает**. В самом `LevelProgressService` даже
нет метода "прочитать текущий прогресс уровня" — только мутации
(`createFirstLevelProgress`, `createNextLevelProgress`, `completeLevel`,
`@Deprecated autoTransitionLevel`).

Практическое следствие, реализованное в `GamePage.tsx`/`CodeSubmitForm.tsx`
как осознанное ограничение, а не недосмотр: команда во время игры **не
видит**, на каком уровне находится, ни его названия, ни легенды, ни
сколько времени осталось до автоперехода. Единственная обратная связь —
результат отправки кода (`remainingMainCodes`/`levelCompleted` из
`CodeSubmissionResponse`) и список уже показанных подсказок. Это
работает как "вслепую вводить коды", а не как полноценный игровой
интерфейс — функционально, но далеко от того, что задумано в
`design-system.md`/`screens.md` для игрового экрана.

Решение — добавить `GET /api/quests/progress/{questId}/{teamId}/level`
(или аналог), возвращающий `LevelProgressResponse` текущего активного
уровня команды. Занесено в `roadmap/backlog.md` как самый приоритетный
...
5. ~~Game Mode~~ — 🟡 частично готово (вход, ввод кода, показанные подсказки — на реальном API, не на моках). **Не хватает** отображения текущего уровня и таймера автоперехода (раздел 4.7) — самый приоритетный оставшийся пробел именно здесь, не абстрактное "ждать backend". Следующий шаг по Game Mode — когда появится нужный эндпоинт, а не отдельная фронтовая задача.
6. ~~Profile~~ — 🟡 готово в рамках возможного (раздел 4.3): личная информация (username/role — без обходных путей, publicName/email/createdAt — email и дата резолвятся тем же честным способом, что и userId в Team/Author, с явным "недоступно" при неоднозначности), авторские квесты. История/статистика — не реализованы, зависят от Statistics (§4.1) и отдельного endpoint'а истории (§4.4).
